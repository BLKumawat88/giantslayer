...
class ApiClient {
  ...
  Future request<T>(Config config, {bool autoLogin = false}) async {
    ...
    return await _processError(_response, config,
        onAutoLoginSuccess: () => request<T>(config));
  }
...
class PenkalaError {
  ...
  PenkalaError(this.response, this.config, {required this.errorType});
  ...
  Future<Null> _processError() async {
    ...
    print("Decoded  Data $_responseData");
    final errorJson = jsonDecode(_responseData); // Creating new Stream session

    print("Session Response $errorJson");
  ...
  static Future<PenkalaError> parseError(
    ...
    final PenkalaError error =
        PenkalaError(response, config, errorType: ErrorType.unauthorized);
