...
import 'package:giantslayer/utils/ApiClient.dart';
import 'package:giantslayer/utils/RequestConfig.dart';
import 'package:giantslayer/utils/rtc_ice_candidate.dart';
...
class Signaling {
  Signaling(this._url, this._secret, this._userName, this._iceServer)
      : _iceServers = {
          "sdpSemantics": "plan-b", // Add this line// Add this line
          'iceServers': [
            {
              'url': 'stun:$_iceServer',
  ...
  Future<dynamic> createWebRtcSession({String? sessionId}) {
    ...
    print("Session12345 'https://$_url/api/sessions'");
    return ApiClient()
  ...
  Future<dynamic> createWebRtcToken(
      {required String sessionId, String role = 'PUBLISHER'}) {
    final Map<String, dynamic> bodyMap = <String, dynamic>{
      'session': sessionId,
      'role': role,
      'data': ''
    ...
    return ApiClient()
        ...
        .then((jsonResponse) {
      print(
          '◤◢◤◢◤◢◤◢◤◢◤ Create WebRTC token POST response: $jsonResponse ◤◢◤◢◤◢◤◢◤◢◤');
  ...
  Future<void> connect() async {
    try {
      _socket = await WebSocket.connect('wss://$_url');
  ...
  Future<void> onMessage(message) async {
    ...
    print(
        '◤◢◤◢◤◢◤◢◤◢◤ onMessage_json_from_socket ---> | $message | ◤◢◤◢◤◢◤◢◤◢◤');
  ...
  void _participantJoinedMethod(Map<String, dynamic> params) {
    ...
    onParticipantsJoined!(remoteParticipant);
    _createRemotePeerConnection(remoteParticipant);
    if (onStateChange != null) {
      onStateChange!(SignalingState.CallStateConnected);
    }
  ...
  void _participantLeftMethod(Map<String, dynamic> params) {
    ...
    if (participantId == _userId) {
      ...
      onParticipantsRemove!(_participants[participantId]);
      _participants[participantId]!.peerConnection?.close();
      _participants.remove(participantId);
      if (_participants.length == 0) {
        if (onStateChange != null) {
          onStateChange!(SignalingState.CallStateWaiting);
        }
      }
    }
  }
  ...
  Future<MediaStream> createStream({isLocalStream = false}) async {
    ...
    if (isLocalStream && onLocalStream != null) {
  ...
  Future<RTCPeerConnection> _createRemotePeerConnection(
    ...
    remotePeerConnection.onAddStream = ((stream) {
      ...
      if (onParticipantsStreamUpdate != null) {
        onParticipantsStreamUpdate!(remoteParticipant);
      }
  ...
  int _sendJson(String method, params) {
